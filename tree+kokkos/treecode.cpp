    	...
    	for ( i = ibeg; i <= iend; i++ ) {
			...
			// printf("test 1-1 loop %d\n", i);
      		...
      		for ( j = ibeg; j < i; j++ ) {
        		...
        		matrixA1D[(i-ibeg)*2*nrow2		+j-ibeg] = -L1*area;
        		matrixA1D[(i-ibeg)*2*nrow2		+j+nrow-ibeg] = -L2*area;
        		matrixA1D[(i+nrow-ibeg)*2*nrow2	+j-ibeg] = -L3*area;
        		matrixA1D[(i+nrow-ibeg)*2*nrow2	+j+nrow-ibeg] = -L4*area; 
      		...
      		matrixA1D[(i-ibeg)*2*nrow2			+i-ibeg] = pre1;
      		matrixA1D[(i+nrow-ibeg)*2*nrow2	+i+nrow-ibeg] = pre2;
		...
		int flag = 0; //yang
	  	...
	  	for ( ii = 0; ii < nrow2; ii++ ) {
	   		...
	   		for (kk = ii; kk < nrow2; kk++){
	   	  		...
	   	  		if ((absA = fabs(matrixA1D[kk*2*nrow2+ii])) > maxA) {	   			
	   	   			maxA = absA;
	   	    		imax = kk;
	   			}
	   	  	}
	   		...
	   		if (imax != ii) {
			   	...
			   	//   	matrixA1D[imax*2*maxparnode+jj] = ptr[jj];	
		   	  	...
		   	  	for (jj = 0; jj < 2*nrow2; jj++){ //0707 maxparnode to nrow2
		   	  		ptr[jj] = matrixA1D[ii*2*nrow2+jj];
			   	  	matrixA1D[ii*2*nrow2+jj] = matrixA1D[imax*2*nrow2+jj];
			   	  	matrixA1D[imax*2*nrow2+jj] = ptr[jj];	
		   	  	}
	   		...
	   		// }
	   		for (jj = ii + 1; jj < nrow2; jj++) { 
	   	  		matrixA1D[jj*2*nrow2	+ii] /= matrixA1D[ii*2*nrow2 +ii];	
	   	  		for (kk = ii + 1; kk < nrow2; kk++){
	   	  	 		matrixA1D[jj*2*nrow2+ kk] -= matrixA1D[jj*2*nrow2+ii] * matrixA1D[ii*2*nrow2+kk];
	   	  		}
	   		}
		...
		int iii, kkk ;
	  	for (iii = 0; iii < nrow2; iii++) {
	   		...
	   		for (kkk = 0; kkk < iii; kkk++){
	      		...
	      		xtemp[iii] -= matrixA1D[iii*2*nrow2 +kkk] * xtemp[kkk];	   
	  	...
	  	for (iii = nrow2 - 1; iii >= 0; iii--) {
	    	...
	    	xtemp[iii] = xtemp[iii] / matrixA1D[iii*2*nrow2 +iii];	    	
	  	}
